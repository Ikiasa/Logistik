...
-- Table: orders
ALTER TABLE orders
    ADD COLUMN total_amount BIGINT DEFAULT 0 CHECK (total_amount >= 0),
    ADD COLUMN total_currency CHAR(3) DEFAULT 'USD' CHECK (total_currency ~ '^[A-Z]{3}$'),
    ADD COLUMN tax_breakdown JSONB DEFAULT '[]'::jsonb; -- Flexible tax structure

-- Table: order_items
ALTER TABLE order_items
    ADD COLUMN unit_price_amount BIGINT DEFAULT 0 CHECK (unit_price_amount >= 0),
    ADD COLUMN unit_price_currency CHAR(3) DEFAULT 'USD' CHECK (unit_price_currency ~ '^[A-Z]{3}$'),
    ADD COLUMN subtotal_amount BIGINT DEFAULT 0 CHECK (subtotal_amount >= 0),
    ADD COLUMN subtotal_currency CHAR(3) DEFAULT 'USD' CHECK (subtotal_currency ~ '^[A-Z]{3}$'),
    ADD COLUMN tax_amount BIGINT DEFAULT 0 CHECK (tax_amount >= 0),
    ADD COLUMN tax_currency CHAR(3) DEFAULT 'USD' CHECK (tax_currency ~ '^[A-Z]{3}$');
